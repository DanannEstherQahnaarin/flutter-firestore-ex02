          ...
          child: ListBody(
            children: [
              ...
              DropdownMenu(
                ...
                dropdownMenuEntries: UserRole.values.map<DropdownMenuEntry<UserRole>>((UserRole option) {
                  return DropdownMenuEntry<UserRole>(
                    ...
                    leadingIcon: option == UserRole.admin ? Icon(Icons.verified_user) : Icon(Icons.person),
                  );
                }).toList(),
              ...
              ElevatedButton(
                onPressed: () {
                  _addMember(context: context, name: _txtNameController.text, email: _txtEmailController.text, password: _txtPasswordController.text, role: _selectedRole);
                },
...
/// 3. 결과에 따른 사용자 피드백 제공
/// 4. 성공 시 다이얼로그 자동 닫기
void _addMember({required String name, required String email, required UserRole role, required String password, required BuildContext context}) async {
  // 이름 입력값 유효성 검사
  // 이름이 비어있으면 에러 메시지 표시 후 함수 종료
  if (name.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Name is Empty')));
    return;
  }
  ...
  final User? user = await AuthService().signUpWithEmail(name: name, email: email, password: password, userRole: role);

  if (user != null) {
    // Firebase에 멤버 추가 요청
    // 비동기 작업이므로 await로 결과 대기
    String result = await addMemberToFirebase(name: name, uid: user.uid, email: email, userRole: role);
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result == Answer.success.name ? 'Member added successfully' : 'Failed to add member')));
